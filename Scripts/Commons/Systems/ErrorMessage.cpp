...
// 初期化処理
void Systems::ErrorMessage::Initialize(const std::wstring& fontName, float displayTime)
{
	m_messageCount = 0;
...
// オブジェクト生成
void Systems::ErrorMessage::CreateObjects(const IComponentManager& iComponentManager)
{
	// テキストの設定
	auto* pRectTransform = m_text.AddComponent<RectTransform>(iComponentManager);
	...
	m_textComponent = m_text.AddComponent<Renderings::Text>(iComponentManager);
